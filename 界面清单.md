...
| 反馈展示 | `#trainingFeedbackArea` | DeepSeek + `Feedback prompts.docx`；落盘 `training_feedback/` |
...
|------|-----------|
| 主逻辑 | `app.py` |
| 知情同意 | `Consent Form - Suspect/Interviewer (Clean).docx` → `/api/consent/` |
| SUE 材料 | `materials/SUE theoretical training group.docx` 等 |
| 反馈 Prompt | `Feedback prompts.docx` → Avatar 培训反馈 |
| 通用案件 | `GENERAL_TERRORISM_CASE_INFO`（`app.py`）、`(CN) Background Information.pdf` |
| 培训反馈存档 | `training_feedback/` 目录（运行后生成） |
| 实验数据 | `experiment_data.xlsx` |
